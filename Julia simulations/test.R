 ...
 #   betaols_var <- rbind(diag(vcov(ols))) 
  ...
  #  lc <- dplyr::select(mybeta_lc, !ends_with("var"))  
   # lc_var <- dplyr::select(mybeta_lc, ends_with("var")) # The estimated variances
    ...
    # lassoiv <- rlassoIV(x = , d = x, z = psiz, y = y,............)
    ...
    # colnames(lassoiv) <-  fgsdfgsdfghsdhfdh
...
# MC_b <- listu[, -1] %>% #[, which(grepl("sample|varu|varv|covuv|fs_fun|IV|DN|CSA|TV|ols|LIML|kclass|np_tsls|WMDF|plug_in|jive", names(listu)) == TRUE)] %>%
#   group_by( sample, varu, varv, covuv, fs_fun, bandwidth ) %>%
#   summarise_all (MSE, parameter = param, type = "bias" )
# 
# MC_v <- listu[, -1] %>% #[, which(grepl("sample|varu|varv|covuv|fs_fun|IV|DN|CSA|TV|ols|LIML|kclass|np_tsls|WMDF|plug_in|jive", names(listu)) == TRUE)] %>%
#   group_by( sample, varu, varv, covuv, fs_fun, bandwidth ) %>%
#   summarise_all (MSE, parameter = param, type = "variance" )
# 
# MC_MSE <- listu[, -1] %>% # #[, which(grepl("sample|varu|varv|covuv|fs_fun|bandwidth|IV|DN|CSA|TV|ols|tsls", names(listu)) == TRUE)] %>%
#   group_by( sample, varu, varv, covuv, fs_fun, bandwidth ) %>%
#   summarise_all (MSE, parameter = param, type = "MSE" )
# 
# MC_medAD <- listu[, -1] %>% # #[, which(grepl("sample|varu|varv|covuv|fs_fun|bandwidth|IV|DN|CSA|TV|ols|tsls", names(listu)) == TRUE)] %>%
#   group_by( sample, varu, varv, covuv, fs_fun, bandwidth ) %>%
#   summarise_all (MSE, parameter = param, type = "MedAD" )
# 
# MC_medbias <- listu[, -1] %>% # #[, which(grepl("sample|varu|varv|covuv|fs_fun|bandwidth|IV|DN|CSA|TV|ols|tsls", names(listu)) == TRUE)] %>%
#   group_by( sample, varu, varv, covuv, fs_fun, bandwidth ) %>%
...
grid <- seq(-1, 3, 0.2) # grid centered around the true parameter beta = 1
